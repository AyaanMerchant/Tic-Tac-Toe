...
user = None  # Represents the user's player selection
board = ttt.initial_state()  # Represents the initial state of the Tic-Tac-Toe board
ai_turn = False  # Indicates whether it's the AI's turn
    ...
    if user is None:
        ...
        if click == 1:
            ...
            if playXButton.collidepoint(mouse):
                ...
                user = ttt.X  # Set user as X
            elif playOButton.collidepoint(mouse):
                time.sleep(0.2)
                user = ttt.O  # Set user as O
        ...
        for i in range(3):
            row = []
            for j in range(3):
                # Create a rectangle for each tile on the board
                rect = pygame.Rect(tile_origin[0] + j * tile_size, tile_origin[1] + i * tile_size, tile_size, tile_size)
                pygame.draw.rect(screen, white, rect, 3)

                # Render and display the move on the board if it's not empty
